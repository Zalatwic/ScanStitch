...
Fresh TESTROLL DNG model-review neutral cleanup on 2026-05-27:

- Current subset artifact: `output/goal_testroll_model_neutral_cleanup_subset_20260527.md`.
- Full-roll artifact: `output/goal_testroll_full_model_neutral_cleanup_20260527.md`.
- Command: `cargo run --release --locked --bin scanstitch-validate -- --roll-dir TESTROLL --roll-suite --roll-suite-frame RAW_0000,RAW_0002,RAW_0004,RAW_0006,RAW_0009,RAW_0011,RAW_0013,RAW_0016 --bit-depth 16 --render-input direct-density --input-mode negative --quality-mode balanced --output-dir output\goal_testroll_model_neutral_cleanup_subset_20260527 --summary-json output\goal_testroll_model_neutral_cleanup_subset_20260527.json --summary-md output\goal_testroll_model_neutral_cleanup_subset_20260527.md --quiet`
- Policy change: `review_model_plausibility` frames now use the existing bounded neutral-highlight, midtone-neutral, and shadow chroma cleanup policy while keeping `tone_color_trust_state=review_required`. This applies cleanup only after neutral and anchor evidence is accepted, but still requires review instead of marking the color decision trusted.
- Focused subset status: `review_required`, with all 8 selected DNG frames rendered and `0` failed frames. The six model-plausibility frames drove the change; the anchor/neutral-support controls were unchanged for the tracked metrics.
- On those six model-plausibility frames, midtone-neutral saturation p95 mean/max improved `0.334322` / `0.338315 -> 0.191019` / `0.193032`, midtone-neutral RGB delta mean/max improved `0.068605` / `0.106922 -> 0.034141` / `0.054300`, and bright-neutral RGB delta mean/max improved `0.098030` / `0.139760 -> 0.060418` / `0.082210`.
- Full 17-frame rerun rendered every DNG with `0` failed frames. Relative to the high-range shadow-lift full run, midtone p50 mean/min/max/range stayed `0.419166` / `0.305494` / `0.544906` / `0.239412`, post-tone high clipping stayed `0.000000`, preserved-gamut minimum stayed `0.978597`, chroma residual p95 mean improved slightly `0.056084 -> 0.055721`, and flat chroma residual p95 mean improved `0.030023 -> 0.029798`.
- Non-TESTROLL guards: `output/validation/logan_model_neutral_cleanup_strict_20260527.md` stayed strict-comparable against `tests/fixtures/baselines/logan_summary_baseline.json` with `summary_baseline_status=comparable` and `issues=[]`; `output/validation/old_testroll_model_neutral_cleanup_raw0055_20260527.md` compared `OLD_TESTROLL` `RAW_0055.dng` against `output/validation/old_testroll_chromadamp055_raw0055_20260523.json` with `comparison.status=comparable` and `issues=[]`. The OLD_TESTROLL frame remained `candidate_risk=safe` and `tone_color_trust_state=trusted`.

Fresh TESTROLL DNG highlight-neutral bridge on 2026-05-27:

- Focused subset artifact: `output/goal_testroll_highlight_neutral_bridge_strong_subset_20260527.md`.
- Full-roll artifact: `output/goal_testroll_full_highlight_neutral_bridge_20260527.md`.
- Command: `cargo run --release --locked --bin scanstitch-validate -- --roll-dir TESTROLL --roll-suite --roll-suite-frame RAW_0000,RAW_0004,RAW_0007,RAW_0008,RAW_0010,RAW_0011,RAW_0016 --bit-depth 16 --render-input direct-density --input-mode negative --quality-mode balanced --output-dir output\goal_testroll_highlight_neutral_bridge_strong_subset_20260527 --summary-json output\goal_testroll_highlight_neutral_bridge_strong_subset_20260527.json --summary-md output\goal_testroll_highlight_neutral_bridge_strong_subset_20260527.md --quiet`
- Policy change: near-neutral highlight cleanup now starts at luminance `0.66` and reaches full strength at `0.86` instead of `0.72` / `0.92`, extending the existing neutral-gated cleanup into the upper-bright band that still carried visible cast. Saturated bright colors remain protected by the existing `0.35` saturation gate.
- Focused subset status: `review_required`, with all 7 selected DNG frames rendered and `0` failed frames. Relative to the model-review neutral cleanup baseline, bright-neutral RGB delta mean/max improved `0.061604` / `0.135182 -> 0.051209` / `0.132486`; `RAW_0008.dng` improved `0.106305 -> 0.063328`, `RAW_0011.dng` improved `0.064935 -> 0.042372`, and `RAW_0010.dng` improved `0.059323 -> 0.048652`. `RAW_0007.dng` remains the bright-neutral outlier, but it moved slightly `0.135182 -> 0.132486`.
- Full 17-frame rerun rendered every DNG with `0` failed frames. Relative to the model-review neutral cleanup full run, midtone p50 mean/min/max/range stayed `0.419166` / `0.305494` / `0.544906` / `0.239412`, bright-neutral RGB delta mean/max improved `0.053155` / `0.135182 -> 0.044968` / `0.132486`, bright-neutral saturation p95 mean improved `0.296916 -> 0.292591`, post-tone high clipping stayed `0.000000`, preserved-gamut minimum stayed `0.978597`, chroma residual p95 mean improved `0.055721 -> 0.055412`, and flat chroma residual p95 mean improved `0.029798 -> 0.029602`.
- Tradeoff audit: two already-low bright-neutral RGB deltas increased, `RAW_0000.dng` `0.012829 -> 0.021064` and `RAW_0005.dng` `0.026384 -> 0.037080`, while remaining below the previous roll mean and without clipping, gamut loss, or residual regressions.
- Non-TESTROLL guards: `output/validation/logan_highlight_neutral_bridge_strict_20260527.md` stayed strict-comparable against `tests/fixtures/baselines/logan_summary_baseline.json` with `summary_baseline_status=comparable` and `issues=[]`; `output/validation/old_testroll_highlight_neutral_bridge_raw0055_20260527.md` compared `OLD_TESTROLL` `RAW_0055.dng` against `output/validation/old_testroll_chromadamp055_raw0055_20260523.json` with `comparison.status=comparable` and `issues=[]`. The OLD_TESTROLL frame remained `candidate_risk=safe` and `tone_color_trust_state=trusted`; bright-neutral RGB delta improved `-0.041091` versus that baseline.

Focused tests run after the audit, anchor-support shadow cleanup, weak-neutral highlight cleanup, saturated-shadow denoise relaxation, shadow-heavy midtone lift, tightened shadow guard, model-review neutral cleanup, and highlight-neutral bridge:
...
Result: targeted density/tone/colorspace/validation tests, full `cargo test --locked`, `cargo clippy --locked --all-targets -- -D warnings`, and `cargo fmt --check` passed on 2026-05-27. The successful test and clippy runs used `CARGO_TARGET_DIR=C:\Users\kszmyd\AppData\Local\Temp\scanstitch-cargo-target-test` and `RUSTFLAGS=-C debuginfo=0` to keep build artifacts off the nearly full validation volume. Coverage includes robust density Dmax selection, direct-density normalization, direct-density fallback selection/rejection, gamut-safe blend selection, shadow/low-range negative tone placement, highlight chroma repair, near-neutral highlight cleanup, upper bright-neutral cleanup, weak-neutral bounded highlight cleanup, anchor/model-review bounded neutral/shadow cleanup, grain diagnostics/denoise behavior, and report validation.
