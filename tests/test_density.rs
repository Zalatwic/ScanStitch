...
fn test_base_subtraction_in_density() {
    ...
    let base_d: Vec<f64> = base_t.iter().map(|&t| scanstitch::density::transmittance_to_density(t)).collect();
    let pixel_t = [0.5, 0.3, 0.15];
    let pixel_d: Vec<f64> = pixel_t.iter().map(|&t| scanstitch::density::transmittance_to_density(t)).collect();
    let sub: Vec<f64> = pixel_d.iter().zip(base_d.iter()).map(|(d, db)| d - db).collect();
    for &s in &sub {
...
fn test_full_phase3_pipeline() {
    ...
    for y in 0..h { for x in 0..w { for ch in 0..c {
        let v = result[[y, x, ch]];
        assert!(v >= 0.0 && v <= 1.0, "out of range: {} at ({},{},{})", v, y, x, ch);
    }}}
}
